...
set _OS_NAME_ARCH     = $_OS_SYSN-$_OS_ARCH
set _OS_NAME_FULL     = $_OS_NAME_ARCH/$_OS_VERS
set _LINUX_VARIANT    = ""
...
if (-r /etc/fedora-release) then
  ...
  if ("$_LINUX_SUBVARIANT" != "") then
     ...
     set _LINUX_SUBVARIANT = FC`cat /etc/fedora-release | sed 's/.*release \(.*\) .*/\1/'`
  endif
  ...
  set _LINUX_SUBVARIANT = RH`cat /etc/redhat-release | sed 's/.*release \(.*\) .*/\1/'`
  set _OS_NAME_FULL = $_OS_SYSN-$_OS_ARCH/$_LINUX_SUBVARIANT-$_OS_VERS
...
cat <<EOF
  where <symbol> must be
    ...
    OS_NAME_ARCH        OS + architecture                                   $_OS_NAME_ARCH
    OS_NAME_FULL        OS [+ OS release] + architecture + version          $_OS_NAME_FULL
